    ...
    var body: some View {
            ZStack{
                ForEach(finishedAnimationCouter...counter, id:\.self){ i in
                    GoodConfettiContainer(animate:$animate[i], finishedAnimationCouter:$finishedAnimationCouter, num:1)
                }
                Button("👍"){
...
struct GoodConfettiContainer: View {
    ...
    var body: some View{
        ZStack{
            ForEach(0...num-1, id:\.self){ _ in
                GoodConfetti(animate: $animate, finishedAnimationCouter:$finishedAnimationCouter)
            }
        }
...
struct GoodConfetti: View{
    @Binding var animate:Bool
    @Binding var finishedAnimationCouter:Int
    @State var movement = GoodMovement(x: 0, y: 0, z: 1, opacity: 0)
...
struct GoodButtonView_Previews: PreviewProvider {
